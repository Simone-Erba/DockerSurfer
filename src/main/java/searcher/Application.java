...
public class Application extends Thread{
	...
	public void start(){
		...
		int pj=0;
		while(ot2.hasNext()) //REMOVE ERRORS
		{
			Node p=ot2.next();
			Relationship r=p.getSingleRelationship(MyRelationshipTypes.DEPENDENCY, Direction.INCOMING);
			System.out.println("deleting "+pj);
			pj++;
			if(r!=null)
			{
				
				Node p2=r.getOtherNode(p);
				if(p2.getProperty("layers").toString().length()>p.getProperty("layers").toString().length())
				{
					System.out.println("deleting between "+p.getProperty("fulltag")+" and "+p2.getProperty("fulltag"));
					r.delete();
	...
	//	System.out.println(nn.getProperty("layers"));
		...
		/*System.out.println("IN"); //REMOVE CYCLES
		Iterable<Relationship> jh=nn.getRelationships(Direction.INCOMING);
		Iterator<Relationship> iy=jh.iterator();
		while(iy.hasNext())
		{
			System.out.println(iy.next().getOtherNode(nn).getProperty("fulltag"));
		}
		System.out.println("OUT");
		Iterable<Relationship> jh3=nn.getRelationships(Direction.OUTGOING);
		Iterator<Relationship> iy3=jh3.iterator();
		while(iy3.hasNext())
		{
			//System.out.println(iy3.next().getOtherNode(nn).getProperty("fulltag"));
		}*/
		...
		while (i.hasNext()) {
			...
			{
				g.insertMap(im,im.getSingleRelationship(MyRelationshipTypes.DEPENDENCY, Direction.INCOMING).getOtherNode(im));
			}
			else
			{
				g.insertMap(im, null);
